...
class TestROC(ExtTestCase):
    ...
    def test_roc(self):
        ...
        fig, ax = plt.subplots()
        self.assertRaise(lambda: test.plot(10, ax=ax, label=[
                         "r10", "p10"], curve=ROC.CurveType.ROC), ValueError)
        ax = test.plot(10, ax=ax, thresholds=True,
                       label=["r10", "p10"], curve=ROC.CurveType.ROC)
        self.assertNotEmpty(ax)
