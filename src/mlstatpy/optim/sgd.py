...
class BaseOptimizer:
    ...
    * *min_threshold*, *max_threshold*: coefficients thresholds
    ...
    def __init__(self, coef, learning_rate_init=0.1,
        ...
        if (min_threshold is not None and
                not isinstance(min_threshold, (float, numpy.float64, numpy.float32))):
            raise TypeError("min_threshold must be a float")
        if (max_threshold is not None and
                not isinstance(max_threshold, (float, numpy.float64, numpy.float32))):
            raise TypeError("min_threshold must be a float")
